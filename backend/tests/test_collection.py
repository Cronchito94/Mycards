...
async def bac_a_sable(client):
    """Suit ce que le test crée et le retire ensuite, quoi qu'il arrive."""

    cree: dict[str, list[int]] = {"items": [], "sales": []}
    yield cree
    for sale_id in cree["sales"]:
        await client.delete(f"/api/v1/collection/sales/{sale_id}")
    for item_id in cree["items"]:
        await client.delete(f"/api/v1/collection/items/{item_id}")
...
class TestAjout:
    async def test_ajout_simple_fusionne(self, client, bac_a_sable) -> None:
        ...
        bac_a_sable["items"].append(item_id)
    ...
    async def test_ajout_avec_prix_cree_un_lot(self, client, bac_a_sable) -> None:
        ...
        sans_prix = await client.post(
            "/api/v1/collection/items", json={"printing_id": pid, "quantity": 1}
        )
        bac_a_sable["items"].append(sans_prix.json()["id"])
...
class TestGroupement:
    async def test_groupe_par_impression_jamais_par_nom(
        ...
        for carte, quantite in zip(cartes, (3, 1), strict=True):
            ...
            r = await client.post(
                "/api/v1/collection/items",
                json={"printing_id": pid, "quantity": quantite},
            )
        ...
        # Deux entrées, aux bonnes quantités : jamais une entrée « Pikachu ×4 ».
        assert par_impression[impressions[0]] == 3
        assert par_impression[impressions[1]] == 1
...
class TestVentes:
    async def test_vente_partielle_decremente(self, client, bac_a_sable) -> None:
        pid = await _une_impression(client)
        r = await client.post(
            "/api/v1/collection/items", json={"printing_id": pid, "quantity": 5}
        )
        item_id = r.json()["id"]
        bac_a_sable["items"].append(item_id)
